  ...
  while [ "$i" -lt 40 ]; do
    ...
    if [ -n "${win}" ]; then
      ...
      docker exec -u default "${container_id}" env DISPLAY=:0 HOME=/home/default TARGET_WIN="$win" sh -lc '
        ...
        wmctrl -ir "$TARGET_WIN" -b add,fullscreen 2>/dev/null || true
        wmctrl -ir "$TARGET_WIN" -e 0,0,0,1920,1080 2>/dev/null || true
        xdotool windowmove "$TARGET_WIN" 0 0 2>/dev/null || true
        xdotool windowsize "$TARGET_WIN" 1920 1080 2>/dev/null || true
        eval "$(xdotool getwindowgeometry --shell "$TARGET_WIN" 2>/dev/null || true)"
        printf "%s|%s" "${WIDTH:-0}" "${HEIGHT:-0}"
      '
      )"
      IFS='|' read -r width height <<< "${result}"
